...
// 🐛 DEBUG TEMPORAL: descomenta para ver qué está llegando
// file_put_contents('/tmp/login_debug.log', "RAW: " . file_get_contents('php://input') . "\n", FILE_APPEND);
...
try 
{
    ...
    // 🔍 DEBUG TEMPORAL: descomenta para ver qué llega
    // error_log("INPUT RAW: [" . $inputRaw . "]");
    
    if (empty($inputRaw)) 
    {
        throw new Exception('El body de la petición está vacío. Raw: [' . $inputRaw . ']');
    }
    ...
    {
        throw new Exception('JSON inválido. Error: ' . json_last_error_msg() . ' | Raw: [' . $inputRaw . ']');
    }

    $usuario         = trim($input['user'] ?? '');
    $clave           = $input['pass'] ?? '';
    $empresa_id      = 0; // $input['empresa_id'] ?? 0;
    $dispo_unique_id = $input['device_id'] ?? ''; // 👈 CAMBIO: string vacío en vez de null
    ...
    // 7️⃣ Registrar en bitácora (solo si hay device_id)
    ...
    // 8️⃣ 🔑 SIEMPRE devolver datos del usuario (FUERA del if)
    echo json_encode([
        'exito' => true,
